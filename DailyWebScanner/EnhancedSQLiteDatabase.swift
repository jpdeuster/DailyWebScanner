...
class EnhancedSQLiteDatabase {
    ...
    func insertLinkRecord(_ linkRecord: LinkRecord) {
        ...
        if sqlite3_prepare_v2(db, sql, -1, &statement, nil) == SQLITE_OK {
            ...
            sqlite3_bind_text(statement, 6, linkRecord.html, -1, nil)
            sqlite3_bind_text(statement, 7, linkRecord.css, -1, nil)
            sqlite3_bind_text(statement, 8, ISO8601DateFormatter().string(from: linkRecord.fetchedAt), -1, nil)
            sqlite3_bind_text(statement, 9, linkRecord.author, -1, nil)
            sqlite3_bind_text(statement, 10, linkRecord.publishDate?.description, -1, nil)
            sqlite3_bind_text(statement, 11, linkRecord.articleDescription, -1, nil)
            sqlite3_bind_text(statement, 12, linkRecord.keywords, -1, nil)
            sqlite3_bind_text(statement, 13, linkRecord.language, -1, nil)
            sqlite3_bind_int(statement, 14, Int32(linkRecord.wordCount))
            sqlite3_bind_int(statement, 15, Int32(linkRecord.readingTime))
            sqlite3_bind_int(statement, 16, Int32(linkRecord.imageCount))
            sqlite3_bind_int(statement, 17, Int32(linkRecord.totalImageSize))
            sqlite3_bind_int(statement, 18, linkRecord.hasAIOverview ? 1 : 0)
            sqlite3_bind_text(statement, 19, linkRecord.aiOverviewJSON, -1, nil)
            sqlite3_bind_text(statement, 20, linkRecord.aiOverviewThumbnail, -1, nil)
            sqlite3_bind_text(statement, 21, linkRecord.aiOverviewReferences, -1, nil)
            sqlite3_bind_int(statement, 22, linkRecord.hasContentAnalysis ? 1 : 0)
            sqlite3_bind_text(statement, 23, linkRecord.contentAnalysisJSON, -1, nil)
            sqlite3_bind_text(statement, 24, linkRecord.htmlPreview, -1, nil)
